...
try {
    ...
    while (($row = fgetcsv($handle, 1000, $delimitador)) !== FALSE) {
        ...
        // Fix encoding if needed (CSV do excel BR costuma ser ISO-8859-1)
        $row = array_map(function($val) {
            return mb_check_encoding($val, 'UTF-8') ? $val : utf8_encode($val);
        }, $row);

        // Mapeamento: Localiz.;N° invent.;Dt.incorp.;Denominação do imobilizado;Marca;ValAquis.
        ...
        if ($stmt_insert) {
            ...
            $erroCount++;
            $mensagensErro[] = "Linha " . ($index + 1) . ": Erro ao preparar banco - " . $conn_manutencao->error;
        }
    }
...
if (isset($conn_manutencao)) $conn_manutencao->close();
