        ...
        <div class="page-actions-bar">
            <form action="" method="GET" class="page-search-form">
                ...
                <div class="page-search-box <?php echo $busca_atual ? 'has-content' : ''; ?>">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($busca_atual); ?>" placeholder="Pesquisar aluno...">
                    <?php if ($busca_atual): ?>
            ...
            <button class="btn-page-action" onclick="showModal('adicaoAluno')"><i class="bi bi-plus-circle"></i> Adicionar Aluno</button>
        </div>
            ...
            <table class="tabela-main">
                ...
                <tbody id="tabela-alunos-srv">
                    ...
                    if (!empty($busca_atual)) {
                        ...
                        $where .= " AND (a.aluno_nome LIKE '%$termo_seguro%' 
                                   OR a.aluno_matricula LIKE '%$termo_seguro%' 
                                   OR a.aluno_email LIKE '%$termo_seguro%')";
                    }

                    // Query de Contagem
                    $sql_count = "SELECT COUNT(*) as total FROM aluno a $where";
                        ...
                        while ($linha = $resultado->fetch_assoc()) {
                            ...
                            echo "<td>
                                    <div style='display: flex; gap: 5px; justify-content: center;'>
                                        <button class='btnAcao editar' type='button' title='Editar' onclick=\"abrirModalEdicaoAluno(" . $linha['idaluno'] . ", '" . addslashes($linha['aluno_nome']) . "', '" . addslashes($linha['aluno_email']) . "', '" . $linha['aluno_matricula'] . "', " . $linha['turmas_id'] . ")\"><i class='bi bi-pencil-square'></i></button>";

                            if (strtolower($linha['aluno_status']) == 'ativo') {
                                echo "<button class='btnAcao deletar' type='button' title='Desativar' onclick=\"showModal('desativarAluno', " . $linha['idaluno'] . ")\"><i class='bi bi-x-lg'></i></button>";
                            } else {
                                echo "<button class='btnAcao confirmar' type='button' title='Ativar' style='background-color: var(--confirmar);' onclick=\"showModal('ativarAluno', " . $linha['idaluno'] . ")\"><i class='bi bi-check-lg'></i></button>";
                            }

                            echo "<button class='btnAcao deletar' type='button' title='Excluir' style='background-color: red;' onclick=\"showModal('deletarAluno', " . $linha['idaluno'] . ")\"><i class='bi bi-trash'></i></button>
                                    </div>
                                  </td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='6' style='text-align:center; padding:30px; opacity:0.6;'><i class='bi bi-info-circle' style='font-size:1.5rem; display:block; margin-bottom:10px;'></i>Nenhum aluno encontrado.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
