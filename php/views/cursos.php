        ...
        <div class="page-actions-bar">
            <form action="" method="GET" class="page-search-form">
                ...
                <div class="page-search-box <?php echo $busca_atual ? 'has-content' : ''; ?>">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($busca_atual); ?>" placeholder="Pesquisar curso...">
                    <?php if ($busca_atual): ?>
            ...
            <button class="btn-page-action" onclick="showModal('adicaoCurso')"><i class="bi bi-plus-circle"></i> Adicionar Curso</button>
        </div>
            ...
            <table class="tabela-main">
                ...
                <tbody id="tabela-cursos-srv">
                    ...
                    if (!empty($busca_atual)) {
                        ...
                        $where .= " AND (c.curso_nome LIKE '%$termo_seguro%')";
                    }

                    // Query de Contagem
                    $sql_count = "SELECT COUNT(*) as total FROM curso c $where";
                    ...
                    if ($resultado && $resultado->num_rows > 0) {
                        while ($linha = $resultado->fetch_assoc()) {
                            ...
                            echo "<td><span class='$classe'>" . $linha["curso_status"] . "</span></td>";

                            echo "<td>
                                    <div style='display: flex; gap: 5px; justify-content: center;'>
                                        <button class='btnAcao editar' title='Editar' type='button'
                                            onclick=\"abrirModalEdicaoCurso(" . $linha['idcurso'] . ", '" . addslashes($linha['curso_nome']) . "')\">
                                            <i class='bi bi-pencil-square'></i>
                                        </button>";

                            if ($status == 'ativo') {
                                echo "<button class='btnAcao deletar' title='Desativar' type='button'
                                                onclick=\"showModal('desativarCurso', " . $linha['idcurso'] . ")\">
                                                <i class='bi bi-x-lg'></i>
                                            </button>";
                            } else {
                                echo "<button class='btnAcao confirmar' title='Ativar' style='background-color: var(--confirmar);' type='button'
                                                onclick=\"showModal('ativarCurso', " . $linha['idcurso'] . ")\">
                        ...
                        echo "<tr><td colspan='3' style='text-align:center; padding:30px; opacity:0.6;'><i class='bi bi-info-circle' style='font-size:1.5rem; display:block; margin-bottom:10px;'></i>Nenhum curso encontrado.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
