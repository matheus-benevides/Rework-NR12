                ...
                <tbody id="tabela-unidade">
                    ...
                    if (!empty($busca_atual)) {
                        ...
                        $sql = "SELECT * FROM unidade WHERE 
                                unidade_nome LIKE '%$termo_seguro%' OR 
                                unidade_cidade LIKE '%$termo_seguro%' OR 
                                unidade_estado LIKE '%$termo_seguro%' OR
                                unidade_numero LIKE '%$termo_seguro%'";
                    } else {
                        ...
                        while ($linha = $resultado->fetch_assoc()) {
                            ...
                            echo "<td>
                                    <div style='display: flex; gap: 5px; justify-content: center;'>
                                        <button class='btnAcao editar' type='button' onclick=\"showModal('editarUnidade', " . $linha['idunidade'] . ")\"><i class='bi bi-pencil-square'></i></button>
                                        <button class='btnAcao deletar' type='button' onclick=\"showModal('deletarUnidade', " . $linha['idunidade'] . ",'')\"><i class='bi bi-trash'></i></button>
                                    </div>
                                  </td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='5' style='text-align:center; padding:15px;'>Nenhuma unidade encontrada.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
