        ...
        <div class="page-actions-bar">
            <form action="" method="GET" class="page-search-form">
                ...
                <div class="page-search-box <?php echo $busca_atual ? 'has-content' : ''; ?>">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($busca_atual); ?>" placeholder="Pesquisar unidade...">
                    <?php if ($busca_atual): ?>
            ...
            <button class="btn-page-action" onclick="showModal('adicaoUnidade')"><i class="bi bi-plus-circle"></i> Adicionar Unidade</button>
        </div>
            ...
            <table class="tabela-main">
                ...
                <tbody id="tabela-unidade-srv">
                    ...
                    if (!empty($busca_atual)) {
                        ...
                        $where .= " AND (unidade_nome LIKE '%$termo_seguro%' 
                                   OR unidade_cidade LIKE '%$termo_seguro%' 
                                   OR unidade_estado LIKE '%$termo_seguro%')";
                    }

                    // Query de Contagem
                    $sql_count = "SELECT COUNT(*) as total FROM unidade $where";
                    $total_resultado = $conn->query($sql_count);
                    $total_registros = $total_resultado->fetch_assoc()['total'];
                    $total_paginas = ceil($total_registros / $registros_por_pagina);

                    // Query Principal com Paginação
                    $sql = "SELECT idunidade, unidade_nome, unidade_cidade, unidade_estado, unidade_numero, unidade_status 
                            FROM unidade 
                            $where 
                    ...
                    if ($resultado && $resultado->num_rows > 0) {
                        while ($linha = $resultado->fetch_assoc()) {
                            ...
                            echo "<td>
                                    <div style='display: flex; gap: 5px; justify-content: center;'>
                                        <button class='btnAcao editar' title='Editar' type='button' 
                            ...
                            if ($status == 'ativo') {
                                echo "<button class='btnAcao deletar' title='Desativar' type='button' onclick=\"showModal('desativarUnidade', " . $linha['idunidade'] . ")\"><i class='bi bi-x-lg'></i></button>";
                            } else {
                                echo "<button class='btnAcao confirmar' title='Ativar' type='button' style='background-color: var(--confirmar);' onclick=\"showModal('ativarUnidade', " . $linha['idunidade'] . ")\"><i class='bi bi-check-lg'></i></button>";
                            }
                        ...
                        echo "<tr><td colspan='6' style='text-align:center; padding:30px; opacity:0.6;'><i class='bi bi-info-circle' style='font-size:1.5rem; display:block; margin-bottom:10px;'></i>Nenhuma unidade encontrada.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
